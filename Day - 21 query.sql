...
cte_rates AS (
   SELECT 
       service,
       ROUND(total_patients_admitted::numeric / total_patients_request, 2) AS admission_rate,
       ROUND(total_patients_refused::numeric / total_patients_request, 2) AS refusion_rate,
...
SELECT 
   s.service, s.total_staff, p.total_patients, p.average_satisfaction
FROM staff_cte s
JOIN patient_cte p 
...
(total admissions, refusals, avg satisfaction), 2)Staff metrics per service 
(total staff, avg weeks present), 3)Patient demographics per service (avg age, count). Then combine
all three CTEs to create a final report showing service name, all calculated metrics, and an overall 
...
SELECT s.service, s.total_admissions, s.total_refusals, s.avg_satisfaction,
    ...
    round((s.avg_satisfaction * 2),2) + s.total_admissions AS performance_score
